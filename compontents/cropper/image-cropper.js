  ...
  methods: {
    ...
    upload() {
      ...
      wx.chooseImage({
        ...
        success(res) {
          ...
          app.globalData.UTIL.an_translateY_out_in(that, { animation: 'animation', show: 'btnsShow' }, true);
        }
      })
    },
    /**
    ...
    _click(event, cb, stop) {
      ...
      this._draw(()=>{
        let x, y;
        if(event) {
          x = event.detail ? event.detail.x : event.touches[0].clientX;
          y = event.detail ? event.detail.y : event.touches[0].clientY;
        }
        if (cb || (x >= this.data.cut_left && x <= (this.data.cut_left + this.data.width)) && (y >= this.data.cut_top && y <= (this.data.cut_top + this.data.height))) {
          //生成图片并回调
          wx.canvasToTempFilePath({
            width: this.data.width * this.data.export_scale,
            height: Math.round(this.data.height * this.data.export_scale),
    ...
    btnClick(e) {
      ...
      if(item.key == 'save') {
        this._click(false, (res) => {
          item = { ...item, ...res };
          that.triggerEvent('btnClick', item)
        }, true);
        return;
      }
